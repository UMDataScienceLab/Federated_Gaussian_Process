...
# auxillary functions for FGPR including optimizer and others
...
calc_rbf_cov_mat <- function(X, lscale, Y=NULL, multi_length=FALSE){
  ...
  if(multi_length==TRUE){
    ...
    scaled_X <- t(t(X)/lscale)
    if (is.null(Y)) scaled_Y <- NULL else scaled_Y <- t(t(Y)/lscale)
    dist_sq_mat <- calc_dist_sq_mat(scaled_X, scaled_Y)
    rbf_cov_mat <- exp(-0.5*dist_sq_mat)
...
calc_rbf_cov <- function(X, lscale, Y=NULL, multi_length=FALSE){
  # Calculate the covariance matrix of an RBF kernel
  # X: input matrix or vector, each row is an observation
  # lscale: lengthscale, all input dimension share a single lengthscale
...
calc_nll <- function(X, y, lscales, oscales, noise_var, kernel="rbf", p=1){
  ...
  u_mat <- chol(cov_mat + diag(0.00, dim(cov_mat)) )
  inv_cov_mat <- chol2inv(u_mat + diag(0.00, dim(cov_mat)) )
...
calc_nll_grad <- function(X, y, lscales, oscales, noise_var, kernel="rbf", p=1){
  ...
  inv_cov_mat <- chol2inv(chol(cov_mat + diag(0.000, dim(cov_mat)) ))
...
calc_pred_mean <- function(X_pred, X, y, lscales, oscales, noise_var){
  ...
  n <- length(y)
  pred_cov_mat <- mapply(function(a,b) a*calc_rbf_cov(X, b, X_pred), oscales, lscales, SIMPLIFY = F) %>%
  ...
  pred_means <- t(pred_cov_mat)%*%solve(cov_mat, y) %>% as.vector
...
### Need to be modified to accomodate matern kernel.
adam <- function(X, y, n_kers=1, batch_size=1, init_step_size=0.01, 
                 ...
                 beta_1=0.9, beta_2=0.999, epsilon=1e-8, multi_length=FALSE){
  # Adam with reparameterization.
  # X: matrix or vector
  # params = log(1+exp(thetas)) + param_mins 
...
#### sgd1 is for non-federated


### Need to be modified to accomodate matern kernel.
sgd <- function(X, y, n_kers=1, batch_size, init_step_size, param_coefs=NULL,
                n_epochs=1, max_iter=NULL,
                init_lscales = NULL, init_oscales = NULL, init_noise_var = NULL,
                fix_lscale=F, fix_oscale=F, fix_noise_var=F,
                lscale_min=1e-6, oscale_min=1e-6, noise_var_min=1e-4,
  ...
  if (unif_sample==T & nn_sample==T){
    while (iter <= max_iter){
      ...
      step_size <- init_step_size #/ iter^0.6
...
#### sgd2 is for federated learning

sgd2 <- function(X, y, n_kers=1, batch_size, init_step_size, param_coefs=NULL,
                 n_epochs=1, max_iter=NULL,
                 init_lscales = NULL, init_oscales = NULL, init_noise_var = NULL,
                 fix_lscale=F, fix_oscale=F, fix_noise_var=F,
                 lscale_min=1e-6, oscale_min=1e-6, noise_var_min=1e-4,
